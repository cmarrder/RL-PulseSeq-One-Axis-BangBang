...
def plot_job(max_time,
    ...
    fig.suptitle('$\mu = {0}, T = {1}$'.format(chem_pot, temp))
    ...
    axd['B'].set_title('Agent ($N_{{chances}} = {0}, N_{{pulse}} = {1}$)'.format(int(Npc), int(Npa)))
    axd['B'].sharex(axd['C'])
...
if __name__=='__main__':
    ...
    oDir = '/home/charlie/Documents/ml/Filter/job_test'

    initials = np.loadtxt(os.path.join(oDir, 'initials.txt'))

    nPulse = int(initials[0])
    nLocation = int(initials[1])
    tMax = initials[2]
    chemPotential = initials[5]
    temperature = initials[6]
