...
if plotCurves == True:
    ...
    axs[1].plot(freq, UDDFilter, label = "UDD", color = UDDColor)
    axs[1].plot(freq, CPMGFilter, label = "CPMG", color = CPMGColor)
    axs[1].set_ylabel(r"$F(\nu)$")
    axs[1].legend()
    
    axs[2].plot(freq, S*UDDFilter, label = '$\chi_{{UDD}}$ = {:.4e}'.format(UDDOverlap), color = UDDColor)
    axs[2].plot(freq, S*CPMGFilter, label = '$\chi_{{CPMG}}$ = {:.4e}'.format(CPMGOverlap), color = CPMGColor)
